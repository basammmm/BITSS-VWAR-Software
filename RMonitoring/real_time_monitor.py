...
class RealTimeMonitor:
    def _is_installer_process_active(self):
        """Detect if msiexec.exe or setup.exe is running (installer mode)."""
        import psutil
        for proc in psutil.process_iter(['name']):
            try:
                name = proc.info['name'].lower()
                if name in ('msiexec.exe', 'setup.exe', 'install.exe', 'uninstall.exe'):
                    return True
            except Exception:
                continue
        return False
    ...
    def wait_and_scan_file(self, path):
        ...
        now = time.time()
        # Installer-aware: if path is installer-protected and installer process is active, use copy-scan
        if excluded and _reason == 'INSTALLER_PROTECTED':
            if self._is_installer_process_active():
                # Copy file to vault for scan, do not quarantine original
                try:
                    from Scanning.scanvault import copy_to_vault_for_scan
                    vaulted_path, meta_path = copy_to_vault_for_scan(norm_path, event='installer_copy_scan')
                    ...
                    print(f"[WARNING] Installer copy-scan failed for {norm_path}: {e}")
                self._recent_events[norm_path] = now
                return
            else:
                # If not installer process, skip vaulting system files
                self._recent_events[norm_path] = now
                return
